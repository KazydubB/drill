...
//@Deprecated
public class ExceptDistinctPrel extends ExceptPrel {

  public ExceptDistinctPrel(RelOptCluster cluster, RelTraitSet traits, List<RelNode> inputs,
                           boolean checkCompatibility) throws InvalidRelException {
    super(cluster, traits, inputs, false, checkCompatibility);
  }

  @Override
  public Minus copy(RelTraitSet traitSet, List<RelNode> inputs, boolean all) {
    try {
      return new ExceptDistinctPrel(this.getCluster(), traitSet, inputs,
  ...
  @Override // todo: this should be revisited
  public RelOptCost computeSelfCost(RelOptPlanner planner, RelMetadataQuery mq) {
    if (PrelUtil.getSettings(getCluster()).useDefaultCosting()) {
      return super.computeSelfCost(planner, mq).multiplyBy(.1);
    }
    double totalInputRowCount = 0;
    for (int i = 0; i < this.getInputs().size(); i++) { // todo: remove unnecessary 'this' references
