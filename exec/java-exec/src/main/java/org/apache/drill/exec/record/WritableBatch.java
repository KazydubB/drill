...
//        v.load(fmd, cbb.slice(bufferOffset, fmd.getBufferLength()));
  ...
  public static WritableBatch getBatchNoHV(int recordCount, Iterable<ValueVector> vectors, boolean isSV2) {
    ...
    for (ValueVector vv : vectors) {
      ...
      if ("Number of records written".equals(vv.getField().getName())) {
    ...
    // todo: maybe set updateCount here?
    RecordBatchDef.Builder builder = RecordBatchDef.newBuilder().addAllField(metadata).setRecordCount(recordCount)
