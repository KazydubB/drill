...
//
//public class ReplicateRecordBatch extends AbstractSingleRecordBatch<FlattenPOP> {
//  private static final Logger logger = LoggerFactory.getLogger(ReplicateRecordBatch.class);
//
...
//  private final ReplicateMemoryManager replicateMemoryManager;
//
//  private final Replicator.Monitor monitor = new Replicator.Monitor() {
//    @Override
...
//  }
//
//  private class ReplicateMemoryManager extends RecordBatchMemoryManager {
//
//    ReplicateMemoryManager(int outputBatchSize) {
...
//  }
//
//  public ReplicateRecordBatch(ReplicatePOP pop, RecordBatch incoming, FragmentContext context) throws OutOfMemoryException {
//    super(pop, context, incoming);
//
//    // get the output batch size from config.
//    int configuredBatchSize = (int) context.getOptions().getOption(ExecConstants.OUTPUT_BATCH_SIZE_VALIDATOR);
//    replicateMemoryManager = new ReplicateMemoryManager(configuredBatchSize);
//
...
//    }
//    replicator.setFlattenField(vector);
//  }
//
//  @Override
//  protected IterOutcome doWork() {
//    replicateMemoryManager.update();
//    replicator.setOutputCount(replicateMemoryManager.getOutputRowCount());
//
//    int incomingRecordCount = incoming.getRecordCount();
//
//    doAlloc(replicateMemoryManager.getOutputRowCount());
//
//    // we call this in setupSchema, but we also need to call it here so we have a reference to the appropriate vector
//    // inside of the the replicator for the current batch
//    setFlattenVector();
//
//    int childCount = incomingRecordCount == 0 ? 0 : replicator.getFlattenField().getAccessor().getInnerValueCount();
//    int outputRecords = childCount == 0 ? 0: replicator.flattenRecords(incomingRecordCount, 0, monitor);
...
//    }
//
//    replicateMemoryManager.updateOutgoingStats(outputRecords);
...
//    int remainingRecordCount = replicator.getFlattenField().getAccessor().getInnerValueCount() - remainderIndex;
//
...
//    doAlloc(Math.min(remainingRecordCount, replicateMemoryManager.getOutputRowCount()));
//
//    int projRecords = replicator.flattenRecords(remainingRecordCount, 0, monitor);
...
//    }
//
//    replicateMemoryManager.updateOutgoingStats(projRecords);
//  }
//
...
//      RecordBatchSizer.ColumnSize colSize = replicateMemoryManager.getColumnSize(v.getField().getName());
...
//
//    ClassGenerator<Replicator> cg = CodeGenerator.getRoot(
//        Replicator.TEMPLATE_DEFINITION, context.getOptions());
...
//
//    try {
//      replicator = context.getImplementationClass(cg.getCodeGenerator());
//      replicator.setup(context, incoming, this, transfers);
...
//    stats.setLongStat(Metric.INPUT_BATCH_COUNT, replicateMemoryManager.getNumIncomingBatches());
//    stats.setLongStat(Metric.AVG_INPUT_BATCH_BYTES, replicateMemoryManager.getAvgInputBatchSize());
//    stats.setLongStat(Metric.AVG_INPUT_ROW_BYTES, replicateMemoryManager.getAvgInputRowWidth());
//    stats.setLongStat(Metric.INPUT_RECORD_COUNT, replicateMemoryManager.getTotalInputRecords());
//    stats.setLongStat(Metric.OUTPUT_BATCH_COUNT, replicateMemoryManager.getNumOutgoingBatches());
//    stats.setLongStat(Metric.AVG_OUTPUT_BATCH_BYTES, replicateMemoryManager.getAvgOutputBatchSize());
//    stats.setLongStat(Metric.AVG_OUTPUT_ROW_BYTES, replicateMemoryManager.getAvgOutputRowWidth());
//    stats.setLongStat(Metric.OUTPUT_RECORD_COUNT, replicateMemoryManager.getTotalOutputRecords());
//
//    RecordBatchStats.logRecordBatchStats(getRecordBatchStatsContext(),
//        "incoming aggregate: count : %d, avg bytes : %d,  avg row bytes : %d, record count : %d",
//        replicateMemoryManager.getNumIncomingBatches(), replicateMemoryManager.getAvgInputBatchSize(),
//        replicateMemoryManager.getAvgInputRowWidth(), replicateMemoryManager.getTotalInputRecords());
//
//    RecordBatchStats.logRecordBatchStats(getRecordBatchStatsContext(),
//        "outgoing aggregate: count : %d, avg bytes : %d,  avg row bytes : %d, record count : %d",
//        replicateMemoryManager.getNumOutgoingBatches(), replicateMemoryManager.getAvgOutputBatchSize(),
//        replicateMemoryManager.getAvgOutputRowWidth(), replicateMemoryManager.getTotalOutputRecords());
//  }
//
//  @Override
//  public void close() {
...
//  @Override
//  public void dump() {
//    logger.error("FlattenRecordbatch[hasRemainder={}, remainderIndex={}, recordCount={}, replicator={}, container={}]",
//        hasRemainder, remainderIndex, recordCount, replicator, container);
//  }
//}
