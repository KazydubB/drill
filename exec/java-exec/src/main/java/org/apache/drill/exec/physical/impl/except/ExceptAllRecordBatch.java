...
public class ExceptAllRecordBatch extends AbstractBinaryRecordBatch<ExceptAll> {

  private static final Logger logger = LoggerFactory.getLogger(ExceptAllRecordBatch.class);

  private final SchemaChangeCallBack callBack = new SchemaChangeCallBack();
  private ExceptAller exceptAll;
  ...
  private ExceptInputIterator exceptInputIterator;

  public ExceptAllRecordBatch(ExceptAll config, List<RecordBatch> children, FragmentContext context) throws OutOfMemoryException {
  ...
  protected void buildSchema() throws SchemaChangeException {
    ...
    exceptInputIterator = new ExceptInputIterator(leftUpstream, left, rightUpstream, right);
  ...
  public IterOutcome innerNext() {
    try {
      while (true) {
        if (!exceptInputIterator.hasNext()) {
          return IterOutcome.NONE;
        }

        Pair<IterOutcome, BatchStatusWrappper> nextBatch = exceptInputIterator.next();
  ...
  private IterOutcome doWork(BatchStatusWrappper batchStatus, boolean newSchema)
    ...
    if (newSchema) {
      createExceptAller(batchStatus.batch);
    }
    ...
    recordCount = exceptAll.exceptRecords(batchStatus.recordsProcessed, recordsToProcess, 0);
  ...
  private void createExceptAller(RecordBatch inputBatch) throws ClassTransformationException, IOException, SchemaChangeException {
    transfers.clear();
    allocationVectors.clear();

    final ClassGenerator<ExceptAller> cg = CodeGenerator.getRoot(ExceptAller.TEMPLATE_DEFINITION, context.getOptions());
    ...
    for(VectorWrapper<?> vw : inputBatch) {
      ...
      if (container.getSchema().getColumn(index).hasSameTypeAndMode(vvIn.getField())
        ...
        // Insert a cast before the Union operation // todo: revisit
    ...
    exceptAll = context.getImplementationClass(cg.getCodeGenerator());
    exceptAll.setup(context, inputBatch, this, transfers);
  }
        ...
        if (leftField.getType().getMinorType() == rightField.getType().getMinorType()) {
          ...
          if (outputMinorType == null) {
            throw new DrillRuntimeException("Type mismatch between " + leftField.getType().getMinorType().toString() +
                ...
                " on the right side in column " + index + " of UNION ALL"); // todo: change
          }
  ...
  private class ExceptInputIterator implements Iterator<Pair<IterOutcome, BatchStatusWrappper>> {
    private final Stack<BatchStatusWrappper> batchStatusStack = new Stack<>();

    ExceptInputIterator(IterOutcome leftOutCome, RecordBatch left, IterOutcome rightOutCome, RecordBatch right) {
    ...
    public Pair<IterOutcome, BatchStatusWrappper> next() {
      while (!batchStatusStack.isEmpty()) {
        ...
        if (topStatus.prefetched) {
          ...
          IterOutcome outcome = ExceptAllRecordBatch.this.next(topStatus.inputIndex, topStatus.batch);
  ...
  @Override
  public void dump() {
    logger.error("ExceptAllRecordBatch[container={}, left={}, right={}, leftOutcome={}, rightOutcome={}, "
