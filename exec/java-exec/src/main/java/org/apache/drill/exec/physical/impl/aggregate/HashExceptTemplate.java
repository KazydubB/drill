...
//
//public abstract class HashExceptTemplate implements HashExcepter {
//  protected static final Logger logger = LoggerFactory.getLogger(HashExceptTemplate.class);
//
...
////  private ExceptRecordBatch outgoing;
...
//      try {
//        setupInterior(left, right, outgoing, aggrValuesContainer);
...
//    public void setupInterior(@Named("left") RecordBatch left, @Named("right") RecordBatch right, @Named("outgoing") RecordBatch outgoing,
...
//
//    @RuntimeOverridden
//    public void outputRecordValues(@Named("incomingRowIdx") int incomingRowIdx, @Named("outRowIdx") int outRowIdx) throws SchemaChangeException{
//    }
//  }
//
//  @Override
//  public void setup(ExceptAll hashExcepterConfig, HashTableConfig htConfig, FragmentContext context, OperatorContext oContext,
////                    RecordBatch incoming, ExceptRecordBatch outgoing, LogicalExpression[] valueExprs, List<TypedFieldId> valueFieldIds,
//                    RecordBatch left, RecordBatch right, ExceptRecordBatch outgoing, LogicalExpression[] valueExprs, List<TypedFieldId> valueFieldIds,
//                    ClassGenerator<?> cg, TypedFieldId[] keyFieldIds, VectorContainer outContainer, int extraRowBytes) throws SchemaChangeException, IOException, ClassTransformationException {
//    System.out.println("HashExceptTemplate#setup");
//    this.allocator = oContext.getAllocator();
////    baseHashTable = new ChainedHashTable(htConfig, context, allocator, incoming, null /* no incoming probe */, outgoing);
//    baseHashTable = new ChainedHashTable(htConfig, context, allocator, right, left /* no incoming probe */, outgoing);
...
//    this.cg = cg;
//    this.groupByOutFieldIds = keyFieldIds;
...
//
//    System.out.println("HashExceptTemplate#delayedSetup");
//
...
////      updateEstMaxBatchSize(incoming); // todo: not sure whether use left or right
//      updateEstMaxBatchSize(left);
//    }
...
//        htable = baseHashTable.createAndSetupHashTable(groupByOutFieldIds);
...
//      this.batchHolders = new ArrayList<>(); // First BatchHolder is created when the first put request is received.
////    }
//    // Initialize the value vectors in the generated code (which point to the incoming or outgoing fields)
//    try {
//      htable.updateBatches();
//    } catch (SchemaChangeException sc) {
//      throw new UnsupportedOperationException(sc);
//    }
//  }
...
//  private void initializeSetup(RecordBatch left, RecordBatch right) throws SchemaChangeException, IOException {
////    baseHashTable.updateIncoming(newIncoming, null); // after a spill - a new incoming
//    baseHashTable.updateIncoming(right, left); // after a spill - a new incoming
...
//    currentBatchRecordCount = left.getRecordCount(); // first batch in this spill file
...
////      htable.updateIncoming(newIncoming.getContainer(), null);
//      htable.updateIncoming(right.getContainer(), left);
////      htables[i].reset();
//      htable.reset();
...
////
////    System.out.println("HashExceptTemplate#doWork");
////
////    int i = -1;
////
////    while (true) {
...
////
////      i++;
////      RecordBatch incoming = i == 0 ? left : right; // todo: this was absent, instance field `incoming` was used instead
////
...
////        }  // need to retry this row (e.g. we had an OOM)
////        else { // next time continue with the next incoming row
////          incIndex();
////        }
////
...
////          outcome = outgoing.next(i, incoming);
//////        }
...
////  }
//
//  // todo: method above was prior to this
//  @Override
//  public AggOutcome doWork() {
//
//    System.out.println("HashExceptTemplate#doWork");
//
//      RecordBatch incoming = right;
//
//      // This would be called only once - first time actual data arrives on incoming
//      if (schema == null && incoming.getRecordCount() > 0) {
//        this.schema = incoming.getSchema();
...
//        // Get the next RecordBatch from the incoming (i.e. upstream operator)
//        // outcome = outgoing.next(i, incoming);
//        outcome = outgoing.next(1, incoming); // todo: I have no idea what this does
////        }
//        long memAllocAfterNext = allocator.getAllocatedMemory();
//        long incomingBatchSize = memAllocAfterNext - memAllocBeforeNext;
//
...
//      BatchHolder bh = batchHolders.get(batchHolders.size() - 1);
...
//      htable.setTargetBatchRowCount(newRowCount);
////    }
//  }
//
//  @Override
...
//      if ( writers != null && spillFiles != null) {
////        try {
...
//          spillFiles = null;
////        } catch(IOException e) {
////          logger.warn("Cleanup: Failed to delete spill file {}", spillFiles, e);
////        }
////      }
//    }
...
//  private void reinitPartition() /* throws SchemaChangeException /*, IOException */ {
//    assert htable != null;
//    htable.reset();
//    if (batchHolders != null) {
//      for (BatchHolder bh : batchHolders) {
...
//      batchHolders.clear();
//    }
//    batchHolders = new ArrayList<>(); // First BatchHolder is created when the first put request is received.
//
//    outBatchIndex = 0;
...
//    int currPartSize = batchHolders.size();
...
//  }*/
//
//  private void addBatchHolder(int batchRowCount) {
//
...
//  @Override
//  @Deprecated
//  public AggIterOutcome outputCurrentBatch() { // todo: should change outcomes?
//
//    // Handle the case of an EMIT with an empty batch
//    if ( handleEmit && ( batchHolders == null || batchHolders.size() == 0 ) ) {
...
//        outBatchIndex = 0; // reset, for the next EMIT
//      //}
...
//    ArrayList<BatchHolder> currPartition = batchHolders;
//    int currOutBatchIndex = outBatchIndex;
...
//    this.htable.outputKeys(currOutBatchIndex, this.outContainer, numPendingOutput);
//
//    // set the value count for outgoing batch value vectors
...
//    if (outBatchIndex == currPartition.size()) {
//
////      if ( EXTRA_DEBUG_SPILL ) {
...
////        }
//        outBatchIndex = 0; // reset, for next time
...
//  }*/
//
//  private int getTargetBatchCount() { // todo: must calculate row count
...
//      hashCode = htable.getBuildHashCode(incomingRowIdx); // todo: add count here?
...
//      putStatus = htable.put(incomingRowIdx, htIdxHolder, hashCode, getTargetBatchCount());
//
...
//      throw new OutOfMemoryException("Well, that's complicated", exc);
//    } catch (SchemaChangeException e) {
//      throw new UnsupportedOperationException("Unexpected schema change", e);
//    }
...
//
//        addBatchHolder(getTargetBatchCount());  // allocate a new (internal) values batch
//
...
//        throw new OutOfMemoryException("Oops!", exc);
//      }
