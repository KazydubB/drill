...
<@pp.changeOutputFile name="/org/apache/drill/exec/expr/fn/impl/G${conversion.from}To${conversion.to}.java" />
 ...
 */
@FunctionTemplate(name = "convert${conversion.from}To${conversion.to}", scope = FunctionTemplate.FunctionScope.SIMPLE, nulls = NullHandling.INTERNAL)
public class G${conversion.from}To${conversion.to} implements DrillSimpleFunc {
  ...
  public void eval() {
    if (<#if conversion.from == "NullableVarChar">left.isSet == 0 || </#if>left.start == left.end) {
      out.isSet = 0;
      return;
    }
    out.isSet = 1;
