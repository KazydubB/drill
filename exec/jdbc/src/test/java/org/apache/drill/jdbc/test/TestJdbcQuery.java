...
public class TestJdbcQuery extends JdbcTestQueryBase {
  ...
  public void testResultSetIsNotReturnedSet() throws Exception {
    ...
    try (Connection conn = connect()) {
      ...
      boolean hasResult = s.execute(String.format("alter session set `%s` = false", ExecConstants.FETCH_RESULT_SET));
      ...
      if (s != null) {
        s.execute(String.format("reset `%s`", ExecConstants.FETCH_RESULT_SET));
        s.close();
      }
    }
  }
  ...
  public void testResultSetIsNotReturnedCTAS() throws Exception {
    ...
    try (Connection conn = connect()) {
      s = conn.createStatement();
      s.execute(String.format("alter session set `%s` = false", ExecConstants.FETCH_RESULT_SET));
  ...
  public void testResultSetIsNotReturnedDropView() throws Exception {
    ...
    try (Connection conn = connect()) {
      ...
      stmt.execute(String.format("alter session set `%s` = false", ExecConstants.FETCH_RESULT_SET));
      ...
      if (stmt != null) {
        stmt.execute(String.format("reset `%s`", ExecConstants.FETCH_RESULT_SET));
        stmt.close();
      }
    }
  }
